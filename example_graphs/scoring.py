...
def sparse2graph(x):
 ...
 #   uG = nxG.to_undirected() 

#    nx.write_edgelist( uG, 'blogcatalog.edgelist', data=False) 
    return {str(k): [str(x) for x in v] for k,v in G.iteritems()}

# 0. Files
embeddings_file = "blogcatalog.embeddings"
#embeddings_file="blogcatalog.emb.weights.joint"
#embeddings_file = "blogcatalog.emb.noweights"
#embeddings_file = np.loadtxt('blogcatalog.orbits')
...
#features_matrix = np.delete( embeddings_file, 10312, 0 )
